...
<task type="auto">
  ...
  <action>
Create the core progress calculation logic using decimal.js for precision (install if needed: `npm install decimal.js`):
...
Ensure decimal.js is installed: check package.json, if not present run `npm install decimal.js`.
  </action>
  <verify>
1. Run `npm run build` - no TypeScript errors
2. Verify decimal.js in package.json dependencies
  </verify>
  <done>Calculator module exports calculateWeightedProgress, buildProgressFormula, deriveStatus, countCompletedChildren</done>
</task>

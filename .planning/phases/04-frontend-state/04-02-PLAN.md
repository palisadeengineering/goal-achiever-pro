...
must_haves:
  truths:
    - "Updating a KPI invalidates only related queries, not unrelated visions"
    - "Progress updates appear fast without full page reload"
    - "Loading states clearly indicate when progress is being recalculated"
    - "Cache reflects server response data after mutation completes"
  artifacts:
...
Create helper function updateTreeWithRollup with the following algorithm:
...
function updateTreeWithRollup(
  ...
  function updateNode(node: KpiTreeNode): KpiTreeNode {
    ...
    if (update) {
      // Apply progress update from server
      node.progress = update.newProgress;
      node.status = update.newStatus;
...
- Uses structuredClone() for deep copy (avoids manual recursion for cloning)
- Builds O(1) lookup map to avoid O(n) search per node
- Mutates the clone, not the original cache data
- Handles arbitrary nesting depth via recursion
- Preserves all node properties not in the update
  </action>
  <verify>
TypeScript compiles: `npx tsc --noEmit`
